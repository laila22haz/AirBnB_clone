...
def cmd_parsing(expr):
    ...
    if match:
        groups = match.groups()
        c_name, command, args = (groups[0], groups[1],
                                 groups[3].split(',') if groups[3] else [])
        return ([command, c_name] +
                [el.strip('" ') for el in args if len(args) > 0])
    return []


class HBNBCommand(cmd.Cmd):
    """class that inherits from the cmd.Cmd class"""
    prompt = "(hbnb) "
    ruler = "="
    ...
    def help_update(self):
        """Displays help message for update cmd"""
